...
import {updateQueryFilter} from "../../../../store/actions/userTempDataActions";
...
const StyledSubmitButton = styled(Button)`
    &.MuiButton-root {
        position: absolute;
        top: -233px; // Fix this abomination in the future
        left: 190px;
    }
`
...
function QFFormFilterTabContent(props) {
    const { register, errors, handleSubmit } = useForm();
    const [activeStep, setActiveStep] = React.useState(0);
    const { states, queryMetadata, updateQueryFilter } = props;
    const classes = useStyles();

    const onSubmit = (data) => {
        updateQueryFilter(data);
    }
    ...
    const queryFields = [
        ...
        {  id: "buildingClass", name: "buildingClass", label: "Building Class",
            options: queryMetadata.buildingClasses,
        ...
        { id: "propertyType", name: "propertyType", label: "Property Type",
            options: queryMetadata.subTypes,
...
const mapDispatchToProps = (dispatch) => {
    return {
        updateQueryFilter: (queryFilter) => dispatch(updateQueryFilter(queryFilter))
    }
}

export default connect(mapStateToProps, mapDispatchToProps)(QFFormFilterTabContent);
