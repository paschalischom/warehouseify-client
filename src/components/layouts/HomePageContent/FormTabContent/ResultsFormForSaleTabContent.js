...
function ResultsFormForSaleTabContent(props) {
            ...
            <OuterWrapper>
                        ...
                        <AutoSizer>
                            {({height, width}) => (
                                <FixedSizeList height={height} width={width} itemSize={170}
                                               itemCount={Object.values(results).filter(result => result.status === "For Sale").length}
                                               itemData={{results: Object.values(results).filter(result => result.status === "For Sale"),
                ...
                <SidebarContainer isOpen={isOpen}>
                    ...
                    <StyledMapContainer isOpen={isOpen}>
                        <ListingsMap results={Object.values(results).filter(result => result.status === "For Sale")} selectedRowIndex={selectedRowIndex}/>
                    </StyledMapContainer>
                </SidebarContainer>
            </OuterWrapper>
        </React.Fragment>
...
export default connect(mapStateToProps)(ResultsFormForSaleTabContent);
